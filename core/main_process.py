...
async def run_main_process(
    user_id: str,
    edition_url: str,
    generate_precta: bool = False,
    generate_postcta: bool = False,
    generate_thread: bool = False,
    generate_long_form: bool = False,
    ...
    try:
        ...
        if generate_thread:
            logger.info("Generating thread tweets")
            thread_tweets = await generate_thread_tweets(
                original_content,
                article_link,
                user_config.get("openai_api_key"),
                example_tweet,
            )
            generated_content["thread"] = thread_tweets

        if generate_long_form:
            ...
            generated_content["long_form"] = long_form_tweet
